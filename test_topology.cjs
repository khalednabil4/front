...
https.get('http://127.0.0.1:8000/core/centers-map/', (res) => {
  ...
  res.on('end', () => {
    ...
    // Let's also print the first two regions points to see if they overlap
    if (regions.length >= 2) {
      console.log(`Region ${regions[0].name} points:`, regions[0].map_shape.points);
      console.log(`Region ${regions[1].name} points:`, regions[1].map_shape.points);
    }
    
    console.log("Internal borders:");
    console.log(json.map.internal_borders);
  });
}).on('error', (err) => {
